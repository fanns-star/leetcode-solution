...
class Solution {
            public int maxSatisfied(int[] customers, int[] grumpy, int X) {
                ...
                int maxDiff = notAngry - angry;
                for (int j = X; j < customers.length; j++) {
                    result += grumpy[j] == 0 ? customers[j] : 0;
                    int start = j - X;
                    angry -= grumpy[start] == 0 ? customers[start] : 0;
                    angry += grumpy[j] == 0 ? customers[j] : 0;
                    notAngry = notAngry - customers[start] + customers[j];
                    int diff = notAngry - angry;
